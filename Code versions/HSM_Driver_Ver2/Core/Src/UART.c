...
/*_________________________________________________________________________________________________________________________*/
	void SAVE_DATA(mDATA get_mode){
		...
		if(get_mode == GET_S_DATA && uart_var.saving == 1){		// get speed value
			uart_var.num_data = 100;
			if(uart_var.cnt < uart_var.num_data){
				uart_var.data_1[uart_var.cnt] = (int)(100*control_var.Id_fb);
				uart_var.data_2[uart_var.cnt] = (int)(100*encoder_var.rpm_fb);
				uart_var.data_3[uart_var.cnt] = (int)(1000*control_var.Iq_sp);
				uart_var.data_4[uart_var.cnt] = (int)(1000*control_var.theta_e);
				uart_var.cnt++;
			}
			else uart_var.save_done = 1;
		}
		/*-----------------------------*/
		if(get_mode == GET_P_DATA && uart_var.saving == 1){		// get position value
