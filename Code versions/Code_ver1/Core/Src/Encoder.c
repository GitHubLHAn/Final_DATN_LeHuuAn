...
//========================================================================================================================
	...
	void CAL_SPEED(ENCODER_OBJECT *pEnc){
		...
		if(pEnc->Direction == 0 ){		//  count up
			...
			else pEnc->delta_pul = pEnc->pulse + pEnc->ppr - pEnc->pulse_pre;		// overflow
		}
		if(pEnc->Direction == 1 ){		//  count down
			if(pEnc->pulse <= pEnc->pulse_pre){		
				pEnc->delta_pul = pEnc->pulse_pre - pEnc->pulse;
			}
			else pEnc->delta_pul = pEnc->pulse_pre + pEnc->ppr - pEnc->pulse;		// overflow
		}
...
//========================================================================================================================
	...
	float CAL_THETA(ENCODER_OBJECT *pEnc, uint32_t cnt_k){
		if(cnt_k == 0){				// If cnt_k =0, we have new cycle 
...
//		pEnc->Direction = __HAL_TIM_IS_TIM_COUNTING_DOWN(&htim2);
//		if(pEnc->Direction == 0){
//			pEnc->delta_pul = pEnc->pulse - pEnc->pulse_pre + pEnc->Overflow*pEnc->ppr;
//			pEnc->rpm = (float)pEnc->delta_pul*pEnc->gain;
//			pEnc->Overflow = 0;
//		}
//		else{
//			if(pEnc->Overflow == 0){
...
//			}
//			else{
//				pEnc->delta_pul = (pEnc->ppr - pEnc->pulse) + pEnc->pulse_pre + (pEnc->Overflow - 1)*pEnc->ppr;
//				pEnc->rpm = (float)pEnc->delta_pul*pEnc->gain;
//				pEnc->Overflow = 0;
//			}
//		}
//		pEnc->rpm = LPF_Run(&LPF_speed_var, pEnc->rpm);
//		pEnc->rad_s = (double)pEnc->rpm/9.55;		// quy doi rad/s -> vong/p
//		/*Uoc luong suc phan dien dong
//		pEnc->EBF_M = pEnc->rad_s*hsm_var_1.kPhiConst;
//		pEnc->EBF_L = pEnc->rad_s*hsm_var_2.kPhiConst;*/
//		pEnc->pulse_pre = pEnc->pulse;
...
//					pEnc->delta_pul = pEnc->pulse - pEnc->pulse_pre;
//				}
//				else pEnc->delta_pul = pEnc->pulse + 65535 - pEnc->pulse_pre;		// overflow, when pulse_k < pulse_k_1
//			}
//			/*Counter down*/
//			if(pEnc->Direction == 1 ){		
//				if(pEnc->pulse <= pEnc->pulse_pre){		
//					pEnc->delta_pul = pEnc->pulse_pre - pEnc->pulse;
//				}
//				else pEnc->delta_pul = pEnc->pulse_pre + 65535 - pEnc->pulse;		// overflow, when pulse_k > pulse_k_1
//			}
//			return pEnc->delta_pul*pEnc->gain_pul_the; // Return the mechanical theta 
//		}
//	}
