...
/* USER CODE BEGIN ET */
		typedef enum {
			mSTAND,			//do nothing
			mHOLD, 			// hold the sharp ar present position
		...
		typedef enum {
			OPENLOOP,	
			CLOSE_C_LOOP, CLOSE_CS_LOOP, CLOSE_CSP_LOOP,			//  current OR current+speed OR current+speed+position CLOSELOOP
		}mRUN_MODE;
		/*--------------*/
		typedef enum {
