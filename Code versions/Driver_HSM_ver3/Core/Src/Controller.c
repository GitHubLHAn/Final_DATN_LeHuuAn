...
/*_________________________________________________________________________________________________________________________*/
	...
	void PI_SPEED_Init(PI_OBJECT *pPI, double Ts)
	{
		...
		pPI->Imin = -4;
		pPI->Ts = Ts; 			//20ms
		pPI->Kp = 0.01; 					//0.1*pPI->Imax/3.14;   // 0.5*Imax/Emax 
		pPI->Ti = 0.5; 
		pPI->Ka = pPI->Kp;
...
/*_________________________________________________________________________________________________________________________*/
	...
	{		
		if(mState_var.flag_set_rpm)
			pCon->theta_e = CAL_THETA(&theta_var, pCon->cnt_k);  // theta_electric = 50*theta_mechanical 
		else 
			pCon->theta_e = 0;				// when speedloop has not operated yet 
